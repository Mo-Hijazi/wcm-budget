...
- **Smoke-test status (2026-06-28):** **profile-save → Supabase verified LIVE** (forced first-run wrote `profiles.school`, then real data restored from backup). **Year generation verified by code inspection only** — `generateYearConfigs` is a pure, clamped, school-agnostic function, but it could NOT be triggered live because the boot-load grandfather guard (`index.html` ~line 2952: `if(loaded.setupVersion===undefined) loaded.setupVersion = raw ? SETUP_VERSION : null`) re-sets `setupVersion` whenever saved data exists → `firstRun=false` on any existing account. **Still TODO: one live fresh-run** — owner approved doing it on his own account as a deliberate test step (back up `app_state`+`profiles` to a localStorage key, clear the row to mimic a brand-new user, run onboarding, verify year-gen, restore). Do when convenient.
